...
def main(arg):
    ...
    client_command="\'sudo python do_tests.py good_ppl\'"
    ...
    for node in plnodes.readlines():
        ...
        try:
            print "sudo /usr/bin/ssh -oBatchMode=yes -l uwaterloo_boxMap -i ~/id_rsa "+node.strip()+" "+server_command
            subprocess.Popen("sudo /usr/bin/ssh -oBatchMode=yes -l uwaterloo_boxMap -i ~/id_rsa "+node.strip()+" "+client_command, shell=True)
